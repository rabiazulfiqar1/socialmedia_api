...
#we will create a token and then delete sub field from it
#or we can also test it by creating a JWT that already doesn't have sub field
def test_get_subject_for_token_type_missing_sub():
    ...
    #this options won't be suitable if our JWT have other required fields in future
    # exp = int(time.time()) + 3600 #setting expiration time to 1 hour from now
    # payload = {"exp": exp, "type": "access"} #creating payload without sub field
    # token = jwt.encode(payload, security.SECRET_KEY, algorithm=security.ALGORITHM)
...
def test_get_subject_for_token_type_wrong_type():
    email = "test@example.com"
    # note: we create confirmation token: security.create_confirmation_token(email) but we try to get subject for access token type security.get_subject_for_token_type(token, "access")
    token = security.create_confirmation_token(email)
    with pytest.raises(security.HTTPException) as exc_info:
        security.get_subject_for_token_type(token, "access")
...
def test_password_hashes():
    password= "password"
    assert security.verify_password(password, security.get_password_hash(password))  #verifying same password works
...
async def test_authenticate_user_not_found(registered_user: dict):
    with pytest.raises(security.HTTPException): #the code inside this with block should raise an exception for test to pass
        await security.authenticate_user("wrong@example.com", "1234")
